...
void Gen::NameAllocator::returnName(int name)
{
...
bool Gen::NameAllocator::isReserved(int name) {
    if (allocatedNames.find(name) != allocatedNames.end())
        return true;
    return false;
}

bool Gen::NameAllocator::noNameReserved()
{
    return allocatedNames.size() == 0;
}

void Gen::NameAllocator::setMaxCount(int _maxCount) {
    maxCount = maxCount;
}

int Gen::NameAllocator::lastNameReserved()
{
...
std::unordered_map<int, int>& Gen::NameAllocator::data()
{
